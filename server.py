...
def get_balance():
  ...
  for account in balance_response['accounts']:
    ...
    existing = conn.execute('SELECT * FROM accounts WHERE account_name = ? AND account_nickname = ?', 
                            (name, nickname)).fetchone()
    if existing is None:
      #Create an account with the balance info
      conn.execute('INSERT INTO accounts (account_name, account_nickname, account_type, balance, auth_key) VALUES (?, ?, ?, ?, ?)',
                    (name, nickname, account_type, balance, access_token))
      print('Added account {} {} with {} balance.'.format(name, nickname, balance))
    else:
      ...
      conn.execute('UPDATE accounts SET balance = ?, auth_key = ? WHERE account_name = ? AND account_nickname = ?',
                  (balance, access_token, name, nickname))
